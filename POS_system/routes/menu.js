...
async function queryMenuRows() {
  ...
  try {
    ...
    const missingCol =
      (err.code === "ER_BAD_FIELD_ERROR" && err.sqlMessage && err.sqlMessage.includes("description_es")) ||
      (err.code === "42703" && err.message && err.message.includes("description_es"))
    if (missingCol) {
...
router.post("/", async (req, res) => {
  try {
    ...
    if (schemaHasDescriptionEs !== false) {
      try {
        const rows = await db.query(
          "INSERT INTO menu_items (name, cat, price, description, description_es, emoji, tag, active) VALUES (?, ?, ?, ?, ?, ?, ?, ?) RETURNING id",
          [
            name,
            cat || "Mains",
            Number(price),
    ...
    if (insertId == null) {
      const rows = await db.query(
        "INSERT INTO menu_items (name, cat, price, description, emoji, tag, active) VALUES (?, ?, ?, ?, ?, ?, ?) RETURNING id",
    ...
    const newId = insertId != null ? insertId : null
    if (newId == null) {
